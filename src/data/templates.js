...
export function getTemplateById(id) {
  return ALL_TEMPLATES.find((t) => t.id === parseInt(id));
}
...
export function getRemixableTemplates() {
...
export const CATEGORIES = [
  {
    id: 'trending',
    title: 'Trending Now',
    items: ALL_TEMPLATES.filter((t) => [1, 2, 3, 4, 5, 6].includes(t.id)),
  },
  {
    id: 'new',
    title: 'New Releases',
    items: ALL_TEMPLATES.filter((t) => [7, 8, 9, 10].includes(t.id)),
  },
  {
    id: 'landing',
    title: 'Landing Pages',
...
export function getAllMergedTemplates(builtInOverrides = {}, adminTemplates = []) {
...
export function getTemplateByIdFromAll(id, builtInOverrides = {}, adminTemplates = []) {
  ...
  return [
    {
      id: 'trending',
      title: 'Trending Now',
      items: active.filter((t) => [1, 2, 3, 4, 5, 6].includes(t.id)),
    },
    {
      id: 'new',
      title: 'New Releases',
...
function _instantiateTemplateLegacy(templateId) {
...
const _s = (extra = {}) => ({ locked: false, visible: true, children: [], ...extra });
...
export const SECTION_BLUEPRINTS = {
  ...
  testimonials: (templateName, color) => ({
  ...
  screenshots: (templateName, color) => ({
  ...
  'case-studies': (templateName, color) => ({
  ...
  'social-proof': (templateName, color) => ({
  ...
  'data-tables': (templateName, color) => ({
  ...
  'recent-activity': (templateName, color) => ({
 ...
 */
export function deepCloneWithIds(nodes) {
 ...
 */
function replaceNames(str, baseNames, newName) {
 ...
 */
function reskinNode(node, baseNames, newName, baseColor, newColor) {
  const cloned = { ...node };
...
function reskinStructure(structure, baseNames, newName, baseColor, newColor) {
 ...
 * @returns {Array|null}
 */
export function getTemplateStructure(templateId) {
  return TEMPLATE_STRUCTURES[parseInt(templateId)] || null;
}
