...
function groupContainsTab(group: any, tabId: string) {
...
export default function Admin() {
  ...
  const [deleteApptInput, setDeleteApptInput] = useState<string>('');
  const [cancelConfirm, setCancelConfirm] = useState<boolean>(false);
  const [sidebarOpen, setSidebarOpen] = useState<boolean>(() => window.innerWidth > 768);
  const [showChangePassword, setShowChangePassword] = useState<boolean>(false);
  const [selectedAppointments, setSelectedAppointments] = useState<Set<string>>(new Set());
  const [bulkMode, setBulkMode] = useState<boolean>(false);
  const [biClientId, setBiClientId] = useState<string>('');
  const [showNewApptForm, setShowNewApptForm] = useState<boolean>(false);
  const [newAppt, setNewAppt] = useState<Record<string, string>>({ name: '', email: '', phone: '', service: '', date: '', time: '', message: '', clientId: '' });
  const [clientSearch, setClientSearch] = useState<string>('');
  const [viewedClients, setViewedClients] = useState<Set<string>>(() => {
    const stored = safeGetItem('threeseas_viewed_clients', []);
    return new Set(stored);
  });

  const markClientViewed = (clientId: string) => {
    setViewedClients((prev: Set<string>) => {
  ...
  const filtered = useMemo(() => appointments
    .filter((a: any) => (filterStatus === 'all' ? true : a.status === filterStatus))
    .filter((a: any) => (selectedDate ? a.date === selectedDate : true))
  ...
  const { pendingCount, followUpCount, archivedCount, pipelineCount, pendingClientCount, clientCount, vipCount, onboardingCount, pendingUserCount, unviewedClientCount } = useMemo(() => {
    ...
    const followUpCount = appointments.filter((a: any) => a.followUp && a.followUp.status !== 'completed' && a.followUp.status !== 'archived' && !a.sentToPipeline).length;
    const archivedFollowUpCount = appointments.filter((a: any) => a.followUp && a.followUp.status === 'archived').length;
    const archivedClientCount = clients.filter((c: any) => c.status === 'archived').length;
    const archivedCount = archivedFollowUpCount + archivedClientCount;
    const pipelineCount = prospects.filter((p: any) => !p.closedAt).length;
    const pendingClientCount = clients.filter((c: any) => c.status === 'pending').length;
    const clientCount = clients.filter((c: any) => c.status !== 'pending' && c.status !== 'archived' && (!c.onboarding || c.onboarding.complete)).length;
    const vipCount = clients.filter((c: any) => c.status === 'vip').length;
    const onboardingCount = clients.filter((c: any) => c.onboarding && !c.onboarding.complete && c.status !== 'archived').length;
    const pendingUserCount = users.filter((u: any) => u.status === 'pending').length;
    const unviewedClientCount = clients.filter((c: any) => c.status !== 'pending' && c.status !== 'archived' && !viewedClients.has(c.id)).length;
  ...
  const toggleAppointmentSelection = (id: string) => {
    setSelectedAppointments((prev: Set<string>) => {
      const next = new Set(prev);
  ...
  const selectAllAppointments = () => {
    if (selectedAppointments.size === filtered.length) {
      ...
      setSelectedAppointments(new Set(filtered.map((a: any) => a.id)));
    }
  };
  ...
  const bulkExportSelected = () => {
    const selected = filtered.filter((a: any) => selectedAppointments.has(a.id));
  ...
  const getBadge = (tabId: string): { count: number; color: string } | null => {
    switch (tabId) {
  ...
  const renderSidebarItem = (item: any, indented: boolean = false) => {
    ...
    <div className="page admin-page">
      ...
      <div className="container">
        <div className="admin-stats">
          <div className="admin-stat-card"><CalendarDays size={24} /><div><h3>{selectedDate ? appointments.filter((a: any) => a.date === selectedDate).length : appointments.length}</h3><p>{selectedDate ? `Bookings on ${formatDisplayDate(selectedDate)}` : 'Total Bookings'}</p></div></div>
          ...
          <>
            <div className="admin-controls">
              ...
              <div className="filter-controls">
                ...
                <button className="btn btn-sm btn-outline calendar-export-btn" onClick={() => exportToICal(filtered.filter((a: any) => a.status !== 'cancelled'), selectedDate ? `appointments-${selectedDate}.ics` : 'all-appointments.ics')}>
            ...
            {showNewApptForm && canManageAppointments && (() => {
              const filteredClients = clientSearch.trim()
                ? clients.filter((c: any) => c.status !== 'archived' && c.status !== 'pending' && (
              ...
              const handleSubmitAppt = (e: React.FormEvent) => {
                e.preventDefault();
            ...
            {view === 'kanban' ? (
              <KanbanView
                ...
                users={users.filter((u: any) => u.status === 'approved')}
