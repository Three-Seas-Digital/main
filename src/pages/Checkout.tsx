...
function StripePaymentForm({ template, onSuccess, onError }: StripePaymentFormProps) {
...
export default function Checkout() {
  ...
  const [form, setForm] = useState<Record<string, string>>({ name: '', email: '', phone: '', address: '', city: '', state: '', zip: '' });
  const [processing, setProcessing] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getTemplateImage(String(template.id)).then((blob) => {
  ...
  const handleDownload = async () => {
    try {
      const blob = await getTemplateZip(String(template.id));
      if (!blob) {
