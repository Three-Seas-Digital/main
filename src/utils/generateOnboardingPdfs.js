...
const BRAND_COLOR = [15, 76, 117]; // #0f4c75
...
export async function generateIntakePdf(client, intakeData) {
...
export async function generateProposalPdf(client, tierData, intakeData, proposalOptions = {}, { signatureData, signedAt } = {}) {
  ...
  y = addField(doc, y, 'Payment Terms', PAYMENT_TERMS_MAP[proposalOptions.paymentTerms] || PAYMENT_TERMS_MAP['net15']);
  y += 4;
...
export async function generateWelcomePacketPdf(client) {
  const JsPDF = await getJsPDF();
  const doc = new JsPDF();
...
export async function generateBiDiscoveryPdf(client, intakeData) {
  const JsPDF = await getJsPDF();
  const doc = new JsPDF();
...
export async function generateAllOnboardingPdfs(client, tierData, intakeData) {
