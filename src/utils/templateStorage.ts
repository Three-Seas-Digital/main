...
async function idbPut(storeName: string, key: string, blob: Blob): Promise<void> {
...
async function r2Request(method: string, route: string, body?: BodyInit, contentType?: string): Promise<Response> {
...
export async function storeTemplateZip(templateId: string, blob: Blob): Promise<void> {
...
export async function getTemplateZip(templateId: string): Promise<Blob | null> {
...
export async function deleteTemplateZip(templateId: string): Promise<void> {
...
export async function getAllTemplateZipIds(): Promise<IDBValidKey[]> {
  return idbGetAllKeys(ZIP_STORE);
}

export async function hasTemplateZip(templateId: string): Promise<boolean> {
...
export async function storeTemplateImage(templateId: string, blob: Blob): Promise<void> {
...
export async function syncMetadataToR2(adminTemplates: unknown, builtInOverrides: unknown): Promise<void> {
...
export async function fetchMetadataFromR2(): Promise<{ adminTemplates: unknown[] } | null> {
...
export async function fetchOverridesFromR2(): Promise<Record<string, unknown> | null> {
  if (!isR2Enabled()) return null;
  try {
...
export async function discoverR2Templates(): Promise<Array<{ id: string; hasImage: boolean; [key: string]: unknown }>> {
  if (!isR2Enabled()) return [];
  try {
    ...
    const items: Array<{ id: string; [key: string]: unknown }> = zipData.items || [];

    // Check which templates also have images
    let imageIds = new Set<string>();
    if (imgRes?.ok) {
      const imgData = await imgRes.json();
      imageIds = new Set((imgData.items || []).map((i: { id: string }) => i.id));
    }
