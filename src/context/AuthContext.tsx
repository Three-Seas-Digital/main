...
export const TEMPLATE_TIERS: Record<string, {
  id: string;
  label: string;
  price: number;
  priceMonthly: number;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [users, setUsers] = useState<any[]>(() => safeGetItem(USERS_KEY, []));
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [currentUser, setCurrentUser] = useState<any>(() => safeGetItem(ADMIN_AUTH_KEY, null));
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [currentClient, setCurrentClient] = useState<any>(() => safeGetItem(CLIENT_AUTH_KEY, null));
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [templateUsers, setTemplateUsers] = useState<any[]>(() => safeGetItem(TEMPLATE_USERS_KEY, []));
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [currentTemplateUser, setCurrentTemplateUser] = useState<any>(() => safeGetItem(TEMPLATE_AUTH_KEY, null));
  ...
  useEffect(() => {
    const hasStaff = users.some((u: any) => u.role === 'staff');
    if (hasStaff) {
      setUsers((prev) => prev.map((u: any) => u.role === 'staff' ? { ...u, role: 'developer' } : u));
    }
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/auth/setup`).then(r => r.json()).then((data: any) => {
    ...
    if (token && !currentUser && !isClientSession) {
      authApi.me().then((user: any) => {
        ...
        usersApi.getAll().then((apiUsers: any[]) => {
          setUsers(apiUsers);
        }).catch(() => {
          // Fallback: just add current user
          setUsers((prev) => {
            const exists = prev.some((u: any) => u.id === user.id);
  ...
  const setupAdmin = async (userData: Record<string, any>) => {
    ...
    try {
      ...
      const user = users.find((u: any) => {
        ...
        if (u.password === password) {
          setUsers((prev) => prev.map((x: any) => x.id === u.id ? { ...x, password: hashed } : x));
          return true;
        }
        return false;
      });
  ...
  const register = async (userData: Record<string, any>) => {
    ...
    try {
      ...
      const existsUsername = users.some((u: any) => u.username === userData.username);
      if (existsUsername) return { success: false, error: 'Username already taken' };
      const existsEmail = users.some((u: any) => u.email.toLowerCase() === userData.email.toLowerCase());
  ...
  // User management
  const addUser = (userData: Record<string, any>) => {
    const exists = users.some((u: any) => u.username === userData.username);
  ...
  const updateUser = (id: string, updates: Record<string, any>) => {
    if (updates.username) {
      const exists = users.some((u: any) => u.username === updates.username && u.id !== id);
    ...
    setUsers((prev) =>
      prev.map((u: any) => (u.id === id ? { ...u, ...localUpdates } : u))
    );
  ...
  const approveUser = (id: string, role: string) => {
    setUsers((prev) =>
      prev.map((u: any) => (u.id === id ? { ...u, role, status: 'approved' } : u))
    );
  ...
  const rejectUser = (id: string) => {
    setUsers((prev) =>
      prev.map((u: any) => (u.id === id ? { ...u, status: 'rejected' } : u))
    );
  ...
  // Client auth
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const clientLogin = (email: string, password: string, clientsList: any[]) => {
    const hashed = hashPassword(password);
    let migrateFn: { id: string; hashed: string } | null = null;
    const client = clientsList.find((c: any) => {
  ...
  // Template user auth
  const registerTemplateUser = (userData: Record<string, any>) => {
  ...
  const templateUserLogin = (email: string, password: string) => {
    const hashed = hashPassword(password);
    const user = templateUsers.find((u: any) => {
  ...
  const updateTemplateUserTier = (userId: string, tier: string, subscriptionData: Record<string, any> = {}) => {
    ...
    if (currentTemplateUser?.id === userId) {
      setCurrentTemplateUser((prev: any) => ({
        ...prev,
        tier,
        subscription: {
          ...prev.subscription,
  ...
  const addToFavorites = (templateId: string) => {
    ...
    setTemplateUsers((prev) =>
      prev.map((u: any) => (u.id === currentTemplateUser.id ? updated : u))
    );
    return { success: true };
  };
  ...
  const removeFromFavorites = (templateId: string) => {
    ...
    const updated = {
      ...currentTemplateUser,
      favorites: currentTemplateUser.favorites.filter((id: string) => id !== templateId),
    };
    setCurrentTemplateUser(updated);
    setTemplateUsers((prev) =>
      prev.map((u: any) => (u.id === currentTemplateUser.id ? updated : u))
  ...
  const canDownloadTemplate = (templateTier: string) => {
  ...
  const getRequiredTier = (templateTier: string): string => {
    if (templateTier === 'Starter') return 'free';
    if (templateTier === 'Business' || templateTier === 'Premium') return 'pro';
    return 'enterprise';
  };

  const recordDownload = (templateId: string, templateTier: string) => {
    if (!currentTemplateUser) return { success: false, error: 'Not logged in' };
    const tier = TEMPLATE_TIERS[currentTemplateUser.tier];
