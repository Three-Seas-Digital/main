...
function drawTierBadge(ctx, template) {
...
function drawNameplate(ctx, name, cat, accent) {
...
function drawNovaDashboard(ctx, c) {
...
function drawMedicalPlus(ctx, c) {
...
function drawTechStartup(ctx, c) {
...
function drawEventLaunch(ctx, c) {
  const grad = ctx.createLinearGradient(0, 0, 0, TEX_H);
...
function drawStartupPitch(ctx, c) {
...
function drawProductHunt(ctx, c) {
...
function drawCourseSales(ctx, c) {
  const grad = ctx.createLinearGradient(0, 0, 0, TEX_H);
...
function drawConsultingCo(ctx, c) {
...
function drawAnalyticsPro(ctx, c) {
  ctx.fillStyle = '#030d1a';
  ctx.fillRect(0, 0, TEX_W, TEX_H);
  // Sidebar
...
function drawProjectTracker(ctx, c) {
...
function drawArtisanGallery(ctx, c) {
...
function drawPixelStudio(ctx, c) {
...
function drawDevPortfolio(ctx, c) {
  ...
  skills.forEach((s, i) => {
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.textAlign = 'left';
    ctx.fillText(s[0], 16, sy);
    ctx.fillStyle = 'rgba(255,255,255,0.06)';
    roundRect(ctx, 100, sy - 6, 400, 6, 3); ctx.fill();
    const grad = ctx.createLinearGradient(100, 0, 100 + s[1] * 4, 0);
    grad.addColorStop(0, c);
    grad.addColorStop(1, '#06b6d4');
    ctx.fillStyle = grad;
    roundRect(ctx, 100, sy - 6, s[1] * 4, 6, 3); ctx.fill();
  });
}
...
export function getTemplatePreviewUrl(template) {
  const canvas = renderGradientCanvas(template);
  return canvas.toDataURL('image/png');
}

function renderGradientCanvas(template) {
 ...
 */
export function useGradientTextures(templates) {
