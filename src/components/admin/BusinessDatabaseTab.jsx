...
export default function BusinessDatabaseTab() {
  ...
  const compiledData = useMemo(() => {
    ...
    const audits = clientId ? allAudits.filter((a) => a.clientId === clientId).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)) : [];
    const latestAudit = audits[0] || null;

    let overallScore = null;
    if (latestAudit?.scores) {
      const vals = Object.values(latestAudit.scores).filter((v) => typeof v === 'number' && v > 0);
      overallScore = vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : null;
    }
                          ...
                          <div className="bdb-onboarding-status">
                            ...
                            {compiledData.onboarding.documents && (
                              <div className="bdb-onboard-docs">
                                {Object.entries(compiledData.onboarding.documents).map(([key, doc]) => (
                        ...
                        {compiledData.audits.length > 0 ? (
                          <div className="bdb-audit-list">
                            {compiledData.audits.slice(0, 5).map((audit) => {
                              const vals = audit.scores ? Object.values(audit.scores).filter((v) => typeof v === 'number' && v > 0) : [];
                              const score = vals.length ? (vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(1) : null;
                              return (
