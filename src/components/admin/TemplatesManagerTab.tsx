...
export default function TemplatesManagerTab() {
  ...
  const [editSource, setEditSource] = useState<string | null>(null); // 'built-in' or 'custom'
  ...
  useEffect(() => {
    return () => {
      Object.values(imageUrls).forEach((url: any) => {
  ...
  const handleZipUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      let hasIndex = !!parsedZip.file('index.html');
      if (!hasIndex) {
        const entries = Object.keys(parsedZip.files);
    ...
    if (parsedZip && !form.imageBlob && !form.hasImage) {
      generatePreviewFromZip(parsedZip);
    }
  };

  const generatePreviewFromZip = async (zip: any) => {
    ...
    try {
      ...
      const blob = await new Promise<Blob | null>((resolve) => resized.toBlob(resolve, 'image/jpeg', 0.85));
      const previewUrl = URL.createObjectURL(blob!);
  ...
  const generatePreviewFromStoredZip = async (templateId: string) => {
    setGeneratingPreview(true);
    try {
  ...
  const tierColor = (tier: string): string => TIER_OPTIONS.find((t) => t.value === tier)?.color || '#6b7280';
