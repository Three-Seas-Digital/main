...
function KanbanCard({ task, client, project, canManage }: { task: any; client: any; project: any; canManage: boolean }) {
  ...
  const priorityColors: Record<string, string> = { low: '#9ca3af', normal: '#3b82f6', high: '#f59e0b', urgent: '#ef4444' };
...
function KanbanColumn({ colKey, label, color, items, client, project, canManage }: { colKey: string; label: string; color: string; items: any[]; client: any; project: any; canManage: boolean }) {
...
export default function ProjectBoard({ client: clientProp }: { client: any }) {
  const {
  ...
  const [projectForm, setProjectForm] = useState<Record<string, any>>({ title: '', description: '', startDate: '', dueDate: '', developers: [] });
  const [activeProject, setActiveProject] = useState<string | null>(null);
  const [milestoneForm, setMilestoneForm] = useState<Record<string, string>>({ title: '', dueDate: '' });
  const [showMilestoneForm, setShowMilestoneForm] = useState<boolean>(false);
  const [showCompletionModal, setShowCompletionModal] = useState<boolean>(false);
  const [completionOptions, setCompletionOptions] = useState<Record<string, any>>({
    archive: true,
  ...
  const [showDevDropdown, setShowDevDropdown] = useState<boolean>(false);
  ...
  const getDevById = (id: string) => staffMembers.find((u: any) => u.id === id);
  ...
  if (project) {
    ...
    return (
      <div className="project-detail">
        ...
        {/* Time Tracking */}
        <TimeTracker clientId={client.id} projectId={project.id} taskId={null} />
