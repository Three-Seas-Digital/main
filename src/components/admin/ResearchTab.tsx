...
export default function ResearchTab() {
          ...
          if (stateFips) {
            ...
            if (dataRes.ok) {
              ...
              if (data.length > 1) {
                ...
                const values = data[1];
                const getVal = (col: string): number => {
  ...
  const loadCategoryData = async (category: string, lat: number, lon: number) => {
  ...
  const getItemType = (tags: Record<string, any>, category: string): string => {
    if (category === 'schools') {
  ...
  const getTypeCounts = (items: any[], _category?: string) => {
  ...
  const formatNumber = (num: number | null | undefined): string => num?.toLocaleString() || 'N/A';
  const formatCurrency = (num: number | null | undefined): string => num ? `$${num.toLocaleString()}` : 'N/A';
  const formatPercent = (num: number, total: number): string => total ? `${((num / total) * 100).toFixed(1)}%` : 'N/A';

  const getRaceChartData = (race: any, total: number) => {
  ...
  const getAgeChartData = (ageGroups: any) => {
    if (!ageGroups) return [];
    return [
  ...
  const getCategoryIcon = (id: string) => {
    const icons: Record<string, React.ReactNode> = { demographics: <Users size={16} />, schools: <GraduationCap size={16} />, businesses: <Building2 size={16} />, healthcare: <Activity size={16} />, dining: <Coffee size={16} />, services: <Briefcase size={16} />, recreation: <Trees size={16} />, government: <Landmark size={16} /> };
    return icons[id] || <Globe size={16} />;
  };

  const Coffee = (props: { size?: number }) => <svg xmlns="http://www.w3.org/2000/svg" width={props.size || 24} height={props.size || 24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 8h1a4 4 0 1 1 0 8h-1"/><path d="M3 8h14v9a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4Z"/><line x1="6" x2="6" y1="2" y2="4"/><line x1="10" x2="10" y1="2" y2="4"/><line x1="14" x2="14" y1="2" y2="4"/></svg>;
