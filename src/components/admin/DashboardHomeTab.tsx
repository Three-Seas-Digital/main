...
interface DashboardHomeTabProps {
  onNavigate?: (tab: string, options?: Record<string, any>) => void;
}

export default function DashboardHomeTab({ onNavigate }: DashboardHomeTabProps) {
  const {
  ...
  const getWeekRevenue = (weeksAgo: number = 0): number => {
  ...
  const formatCurrency = (num: number): string => `$${num.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
      ...
      <div className="dashboard-stats-grid">
        ...
        <div className="dashboard-stat-card">
          ...
          <div className="dashboard-stat-content">
            ...
            <span className={`dashboard-stat-sub ${parseFloat(String(revenueChange)) >= 0 ? 'positive' : 'negative'}`}>
              {parseFloat(String(revenueChange)) >= 0 ? '+' : ''}{revenueChange}% vs last week
            </span>
          </div>
        </div>
                  ...
                  <div key={inv.id} className="dashboard-invoice-item">
                    ...
                    <div className="dashboard-invoice-amount">
                      ...
                      <span className="overdue-days">
                        {Math.floor((new Date().getTime() - new Date(inv.dueDate).getTime()) / (1000 * 60 * 60 * 24))} days overdue
                      </span>
                    </div>
                  </div>
                ))}
