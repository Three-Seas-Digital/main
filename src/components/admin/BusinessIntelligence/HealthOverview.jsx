...
export default function HealthOverview({ biClientId, onBiClientChange }) {
  ...
  const gridData = useMemo(() => {
    return activeClients.map(client => {
      const latest = allAudits.filter(a => a.clientId === client.id).sort((a,b) => new Date(b.createdAt)-new Date(a.createdAt))[0];
      const catScores = {};
      if (latest && latest.scores) {
        CATS.forEach(cat => {
          const pre = cat.id.replace('cat-','sc-');
          const vals = Object.entries(latest.scores).filter(([k])=>k.startsWith(pre)).map(([,v])=>v).filter(v=>v>0);
          catScores[cat.id] = vals.length ? (vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1) : '--';
        });
      }
      const allVals = Object.values(catScores).map(v=>parseFloat(v)).filter(v=>!isNaN(v));
