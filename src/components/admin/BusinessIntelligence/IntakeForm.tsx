...
const Section = ({ k, title, icon, expanded, onToggle, children }: { k: string; title: string; icon: React.ReactNode; expanded: boolean; onToggle: (k: string) => void; children: React.ReactNode }) => (
...
const Sel = ({ label, value, onChange, opts }: { label: string; value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; opts: string[] }) => (
...
const Txt = ({ label, value, onChange, type = "text", ph = "" }: { label: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; type?: string; ph?: string }) => (
  <div className="form-group"><label>{label}</label>
...
export default function IntakeForm({ biClientId, onBiClientChange }: IntakeFormProps) {
  const { clients } = useAppContext();
  const [selectedClientId, setSelectedClientId] = useState(biClientId || '');
  const [form, setForm] = useState<Record<string, any>>({ ...EMPTY_INTAKE });
  const [saving, setSaving] = useState(false);
  const [saveMsg, setSaveMsg] = useState('');
  const [expanded, setExpanded] = useState<Record<string, boolean>>({ business: true, digital: true, marketing: false, goals: false, notes: false, kpis: false });
  ...
  const toggleSocial = (s: string) => setForm(p => ({ ...p, social_platforms: p.social_platforms.includes(s) ? p.social_platforms.filter((x: string) => x !== s) : [...p.social_platforms, s] }));
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    const intakes = safeGetItem(INTAKES_KEY, {});
    const data: Record<string, any> = { ...form, updatedAt: new Date().toISOString() };
