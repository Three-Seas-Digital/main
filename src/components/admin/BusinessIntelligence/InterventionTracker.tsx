...
function calcDelta(before: any, after: any) {
...
function roiRating(roi: number): { label: string; color: string } {
...
export default function InterventionTracker({ biClientId, onBiClientChange }: InterventionTrackerProps) {
  ...
  const [form, setForm] = useState<Record<string, any>>(emptyIntervention());
  ...
  const [formSections, setFormSections] = useState<Record<string, boolean>>({ basic: true, desc: false, before: false, after: false, links: false });
  const toggleFormSection = (k: string) => setFormSections((p: Record<string, boolean>) => ({ ...p, [k]: !p[k] }));
  ...
  const persist = (updated: any[], changedItem?: any) => {
  ...
  const addCustomMetric = (field: string) => {
    setForm(prev => ({
      ...prev,
  ...
  const updateCustomMetric = (field: string, idx: number, key: string, val: string) => {
    setForm(prev => {
  ...
  const removeCustomMetric = (field: string, idx: number) => {
    setForm(prev => ({
      ...prev,
  ...
  const setFormField = (field: string, value: any) => setForm((prev: Record<string, any>) => ({ ...prev, [field]: value }));
  const setMetric = (section: string, key: string, val: any) => setForm((prev: Record<string, any>) => ({ ...prev, [section]: { ...prev[section], [key]: val } }));
  ...
  const buildComparisonData = (item: any) => {
  ...
  const buildSnapshotData = (item: any, metricKey: string) => {
    const points = [];
          ...
          {interventions.length > 1 && (
            <div className="bi-section" style={{ marginBottom: 16 }}>
              ...
              <div className="bi-interventions-timeline">
                {[...interventions].sort((a: any, b: any) => new Date(a.startDate || a.createdAt).getTime() - new Date(b.startDate || b.createdAt).getTime()).map((item: any) => {
...
function DeltaBadge({ before, after, invert }: { before: any; after: any; invert?: boolean }) {
  const b = parseFloat(before) || 0;
  const a = parseFloat(after) || 0;
