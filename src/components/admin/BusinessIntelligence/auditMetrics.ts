...
interface Client {
  ...
  invoices?: Array<{ status?: string; createdAt?: string; paidAt?: string }>;
...
export function calcRevenueGrowthRate(payments: Payment[], currentMonth: string, priorMonth: string): number {
...
export function calcAOV(payments: Payment[]): number {
...
export function calcARPU(payments: Payment[], clients: Client[]): number {
...
export function calcRevenueConcentration(payments: Payment[], topN: number = 3): number {
  const completed = payments.filter(p => p.status === 'completed');
  const total = completed.reduce((s, p) => s + (p.amount || 0), 0);
  if (total === 0) return 0;
...
export function calcExistingCustomerMix(payments: Payment[], clients: Client[]): number {
  const completed = payments.filter(p => p.status === 'completed');
  const total = completed.reduce((s, p) => s + (p.amount || 0), 0);
  if (total === 0) return 0;
...
export function calcWinRate(prospects: Prospect[]): number {
...
export function calcAvgDealSize(prospects: Prospect[]): number {
  const won = prospects.filter(p => p.outcome === 'won' && (p.dealValue ?? 0) > 0);
  if (won.length === 0) return 0;
  return won.reduce((s, p) => s + (p.dealValue ?? 0), 0) / won.length;
}

/** Average sales cycle length in days (created -> closed for won prospects) */
export function calcSalesCycleLength(prospects: Prospect[]): number {
  const won = prospects.filter(p => p.outcome === 'won' && p.createdAt && p.closedAt);
  if (won.length === 0) return 0;
  const totalDays = won.reduce((s, p) => {
    const days = (new Date(p.closedAt!).getTime() - new Date(p.createdAt!).getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export function calcLeadConversionRate(leads: Lead[], clients: Client[], prospects: Prospect[] = []): number {
...
export function calcGrossMargin(payments: Payment[], expenses: Expense[]): number {
...
export function calcDSO(clients: Client[]): number {
  const paidInvoices: number[] = [];
  clients.forEach(c => {
    (c.invoices || []).forEach(inv => {
      if (inv.status === 'paid' && inv.createdAt && inv.paidAt) {
        const days = (new Date(inv.paidAt).getTime() - new Date(inv.createdAt).getTime()) / (1000 * 60 * 60 * 24);
...
export function calcRevenuePerFTE(payments: Payment[], users: User[]): number {
...
export function calcProjectCompletionRate(clients: Client[]): number {
  let total = 0;
  let completed = 0;
  clients.forEach(c => {
...
export function calcDataCompleteness(clients: Client[], intakes: Record<string, unknown>): number {
...
export function calcPipelineCoverage(prospects: Prospect[], target: number): number | null {
...
export function calcIFSR(impact: number | string, feasibility: number | string, speed: number | string, risk: number | string): number {
...
export function getIFSRDecision(score: number): string {
...
export function calcTotalRevenue(payments: Payment[], year: number | string): number {
  return payments
...
export function calcGrossProfitPerFTE(payments: Payment[], expenses: Expense[], users: User[]): number {
  const approved = users.filter(u => u.status === 'approved').length;
  if (approved === 0) return 0;
  const revenue = payments
    .filter(p => p.status === 'completed')
...
export function fmtCurrency(num: number | string | null | undefined): string {
  return `$${Number(num || 0).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
}

/** Format percentage */
export function fmtPct(num: number | string | null | undefined, decimals: number = 1): string {
