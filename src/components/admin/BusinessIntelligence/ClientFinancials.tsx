...
const SummaryCard = ({ icon, label, value, color, trendValue }: { icon: React.ReactNode; label: string; value: string; color: string; trendValue?: number | null }) => (
...
const Th = ({ col, label, sortCol, sortDir, onToggleSort }: { col: string; label: string; sortCol: string; sortDir: string; onToggleSort: (col: string) => void }) => (
  <th onClick={() => onToggleSort(col)} style={{ cursor: 'pointer' }}>
    {label} {sortCol === col ? (sortDir === 'asc' ? '\u25B2' : '\u25BC') : ''}
  </th>
);

const YoYRow = ({ label, thisVal, lastVal, change, isCurrency }: { label: string; thisVal: number; lastVal: number; change: number; isCurrency?: boolean }) => (
  <tr>
    <td>{label}</td>
...
export default function ClientFinancials({ biClientId, onBiClientChange }: ClientFinancialsProps) {
  ...
  const set = (f: string, v: any) => setForm((p: Record<string, any>) => ({ ...p, [f]: v }));
