...
function calcNetProfitMargin(payments, expenses) {
...
function computeRiskCategories(clients, payments, prospects, expenses) {
  ...
  const allAudits = Object.values(auditsData).flat();
  const lowScoringCategories = [];
  allAudits.forEach(audit => {
    if (!audit || !audit.categories) return;
    Object.entries(audit.categories).forEach(([cat, data]) => {
  ...
  const allScores = [];
  Object.values(auditsData).flat().forEach(audit => {
...
export default function RiskAssessment({ biClientId, onBiClientChange }) {
