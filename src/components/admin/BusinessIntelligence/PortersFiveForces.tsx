...
function intensityBg(value: number): string {
...
function computeAutoForces({ payments, intakes, clientId }: { payments: any[]; intakes: Record<string, any>; clientId: string }): Record<string, number> {
...
function CustomRadarTooltip({ active, payload }: { active?: boolean; payload?: any[] }) {
...
export default function PortersFiveForces({ biClientId, onBiClientChange }: PortersFiveForcesProps) {
  ...
  const [allData, setAllData] = useState<Record<string, any>>(() => safeGetItem(PORTERS_KEY, {}));
  ...
  const updateForce = useCallback(
    (forceKey: string, value: number | string) => {
      if (!clientId) return;
        ...
        <>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      ...
                      <input
                        ...
                        step={1}
                        value={val}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateForce(force.key, e.target.value)}
          ...
          >
            ...
            <textarea
              ...
              onFocus={(e: React.FocusEvent<HTMLTextAreaElement>) => (e.target.style.borderColor = 'var(--primary)')}
              onBlur={(e: React.FocusEvent<HTMLTextAreaElement>) => (e.target.style.borderColor = 'var(--glass-border)')}
            />
          </div>
