...
export default function ExecutionTracker({ biClientId, onBiClientChange }) {
  ...
  const [allPlans, setAllPlans] = useState(() => safeGetItem(EXEC_KEY, {}));
  ...
  const updateItem = (waveId, itemId, field, value) => {
    if (!activePlan) return;
    const updated = {
      ...activePlan,
      waves: {
  ...
  const deleteItem = (waveId, itemId) => {
    if (!activePlan) return;
    const updated = {
      ...activePlan,
  ...
  const overallProgress = useMemo(() => {
    ...
    Object.values(activePlan.waves).forEach(items => {
      total += items.length;
  ...
  const generateFromKPIs = () => {
    ...
    Object.entries(clientKpis.kpis).forEach(([kpiId, kpi]) => {
      ...
      const label = kpi.customLabel || kpiId.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      const category = KPI_CATEGORIES[kpiId] || 'Operations';
