...
export default function AIAdvisor({ biClientId, onBiClientChange }) {
  const { clients = [] } = useAppContext();

  // Panel state
  const [activePanel, setActivePanel] = useState('configure');
  ...
  // Run analysis
  const handleAnalyze = async (snapshotId) => {
    if (!biClientId) return;
    setAnalyzing(true);
    setError('');
    try {
  ...
  const handleItemAction = async (itemId, status) => {
    ...
    try {
      ...
      setCurrentAnalysis(prev => ({
        ...prev,
        items: prev.items.map(i => i.id === itemId ? { ...i, admin_status: status } : i),
      }));
  ...
  const categories = useMemo(() => {
    if (!currentAnalysis?.items) return [];
    return [...new Set(currentAnalysis.items.map(i => i.category).filter(Boolean))];
  }, [currentAnalysis]);
