...
function makeItem(text, isAuto = false) {
  return { id: generateId(), text, isAuto };
}

function buildAutoSWOT({ clients, payments, prospects, audits, intakes, interventions }) {
  const strengths = [];
  const weaknesses = [];
  const opportunities = [];
  const threats = [];
...
export default function SWOTAnalysis({ biClientId, onBiClientChange }) {
  ...
  const printReport = useCallback(() => {
    ...
    const renderQuadrantHtml = (items, label, color) => {
      const rows = (items || [])
