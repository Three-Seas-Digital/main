...
function categoryAvg(scores, keys) {
  const vals = keys.map(k => scores[k] || 0).filter(v => v > 0);
  return vals.length ? +(vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(1) : 0;
}

function overallFromAudit(audit) {
...
function getDateCutoff(preset, customStart) {
...
function Card({ title, icon, extra, children }) {
  return (
...
export default function ClientAnalytics({ biClientId, onBiClientChange }) {
  ...
  const filterByDate = useCallback((dateStr) => {
  ...
  const clientAudits = useMemo(() => allAudits.filter(a => a.clientId === clientId && filterByDate(a.createdAt)).sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)), [allAudits, clientId, filterByDate]);
  ...
  // Compare client data
  const compareAudits = useMemo(() => compareClientId ? allAudits.filter(a => a.clientId === compareClientId && filterByDate(a.createdAt)).sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)) : [], [allAudits, compareClientId, filterByDate]);
  const compareLatestAudit = compareAudits.at(-1) || null;

  // Radar data
  const radarData = useMemo(() => {
    if (!latestAudit?.scores) return [];
    const items = Object.entries(CATEGORY_MAP).map(([name, cfg]) => ({ category: name, score: categoryAvg(latestAudit.scores, cfg.keys), fullMark: 10 }));
