...
const getPaymentDate = (p: any): Date => new Date(p.createdAt);
const getPaymentYear = (p: any): number => getPaymentDate(p).getFullYear();
const getPaymentMonth = (p: any): number => getPaymentDate(p).getMonth();
const getPaymentQuarter = (p: any): number => Math.floor(getPaymentDate(p).getMonth() / 3);
...
export default function RevenueTab() {
  ...
  const byTier = useMemo(() => {
    const tiers: Record<string, { name: string; label: string; revenue: number; count: number }> = {};
    yearPayments.forEach((p) => {
  ...
  const byMethod = useMemo(() => {
    const methods: Record<string, { name: string; revenue: number; count: number }> = {};
    yearPayments.forEach((p) => {
  ...
  const formatLabel = (str: string): string => str.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
