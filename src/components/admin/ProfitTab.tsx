...
export default function ProfitTab() {
  ...
  const availableYears = useMemo(() => {
    ...
    return Array.from(years as Set<number>).sort((a, b) => b - a);
  }, [payments, expenses]);
  ...
  const formatCurrency = (num: number): string => `$${Math.abs(num).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
