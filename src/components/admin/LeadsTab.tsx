...
function classifyBusiness(tags: Record<string, any>): string {
...
function getBusinessType(tags: Record<string, any>): string {
...
export default function LeadsTab() {
  ...
  const [sendToFollowUpConfirm, setSendToFollowUpConfirm] = useState<any>(null);
  const [sendToPipelineConfirm, setSendToPipelineConfirm] = useState<any>(null);
  const [toastMsg, setToastMsg] = useState('');
  const [selectedResult, setSelectedResult] = useState<any>(null);
  const [enrichData, setEnrichData] = useState<Record<string, any>>({});
  ...
  const handleLeadStatusChange = (lead: any, newStatus: string) => {
  ...
  // Lookup URLs
  const getLookupUrls = (business: any) => {
  ...
  const handleSendToFollowUp = (lead: any) => {
  ...
  const handleSendToPipeline = (lead: any) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setSearchCenter({ lat: parseFloat(lat), lon: parseFloat(lon), name: display_name } as any);
        ...
        {searchCenter && (
          <div className="leads-map-section">
            <div className="leads-map-header">
              <MapPin size={16} />
              <span>Search Area: {(searchCenter as any).name?.split(',').slice(0, 3).join(',')}</span>
