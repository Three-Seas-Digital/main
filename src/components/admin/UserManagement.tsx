...
export default function UserManagement() {
  ...
  const startEdit = (user: any) => { setForm({ username: user.username || '', password: '', name: user.name || '', email: user.email || '', role: user.role || 'developer' }); setEditingId(user.id); setShowForm(true); setError(''); };
  ...
  const availableRoles = useMemo(() => {
    ...
    const filtered = {};
    for (const [key, role] of Object.entries(ROLES) as [string, any][]) {
              ...
              <div key={user.id} className="pending-card">
                ...
                <div className="pending-card-actions">
                  <div className="pending-role-select">
                    ...
                    >
                      {Object.entries(availableRoles).map(([key, role]: [string, any]) => (
