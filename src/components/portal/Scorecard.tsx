...
function ScoreHistoryChart({ history }: { history: Array<{ score: number; label: string; date?: string }> }) {
...
export default function Scorecard() {
  ...
  const overallScore = useMemo(() => {
    const scores = Object.values(latestScoresMap) as any[];
    if (scores.length === 0) return 0;
    const totalWeight = scores.reduce((sum: number, s: any) => sum + (s.weight || 1), 0);
    const weightedSum = scores.reduce((sum: number, s: any) => sum + (s.score || 0) * (s.weight || 1), 0);
