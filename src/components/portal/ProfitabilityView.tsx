...
const pct = (v: any): string => Number(v || 0).toFixed(1) + '%';

function KpiCard({ icon, label, value, color, trend }: { icon: React.ReactNode; label: string; value: string; color: string; trend?: number | null }) {
  return (
...
export default function ProfitabilityView() {
  ...
  const stats = useMemo(() => {
    ...
    const bestQuarter = Object.entries(quarters)
      .map(([key, data]: [string, any]) => ({ key, profit: data.revenue - data.expenses }))
