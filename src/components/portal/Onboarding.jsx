...
/* ===== Signature Pad ===== */
function SignaturePad({ onSign, disabled }) {
...
export default function PortalOnboarding({ client }) {
  ...
  /* ===== Render: Intake Form ===== */
  const renderIntakeForm = (docEntry) => {
  ...
  const renderWelcomePacket = (docEntry) => {
    const statusInfo = DOC_STATUS_COLORS[docEntry.status] || DOC_STATUS_COLORS.pending;
