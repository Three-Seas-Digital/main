-- ============================================================
-- Three Seas Digital — Supabase / PostgreSQL Seed Data
-- Converted from: seed.sql + seed-bi.sql
-- Conversion date: 2026-03-04
-- Target: PostgreSQL 15+ (Supabase)
--
-- Prerequisite: schema.sql must be run first.
--
-- Changes from MySQL source:
--   - gen_random_uuid()::TEXT used for email_template IDs
--     (MySQL used UUID(); PG equivalent is gen_random_uuid())
--   - INSERT IGNORE → INSERT ... ON CONFLICT DO NOTHING
--   - ON DUPLICATE KEY UPDATE → INSERT ... ON CONFLICT (...) DO UPDATE SET
--   - audit_categories seed: display_order column removed (dropped per audit)
--   - audit_subcriteria seed: display_order column removed (dropped per audit)
--   - Single-quoted apostrophes in body text use $$ quoting to avoid escaping
--     (all multi-line strings use standard '' escaping to match PG syntax)
-- ============================================================


-- ============================================================
-- SECTION 1: Email Templates (5 defaults)
-- Source: seed.sql
-- Uses ON CONFLICT DO NOTHING so re-running is safe.
-- IDs generated at insert time using gen_random_uuid().
-- ============================================================

INSERT INTO email_templates (id, name, subject, body, category, is_default)
VALUES
    (
...
-- ============================================================
-- SECTION 2: Audit Categories (6 base categories)
-- Source: seed-bi.sql
-- Uses ON CONFLICT (id) DO NOTHING — safe to re-run.
-- display_order column removed (dropped per audit; sort_order is canonical).
-- ============================================================

INSERT INTO audit_categories
    (id, name, slug, description, is_base, default_weight, sort_order, icon, color)
VALUES
    (
        'cat-web-presence',
        'Web Presence',
        'web-presence',
        'Website design, performance, mobile responsiveness, and user experience',
        TRUE, 25.00, 1, 'Globe', '#3B82F6'
    ),
    (
        'cat-seo',
        'SEO',
        'seo',
        'Search engine optimization, keyword rankings, technical SEO, and local search',
        TRUE, 20.00, 2, 'Search', '#10B981'
    ),
    (
        'cat-social-media',
        'Social Media',
        'social-media',
        'Social media presence, engagement, content strategy, and audience growth',
        TRUE, 15.00, 3, 'Share2', '#8B5CF6'
    ),
    (
        'cat-branding',
        'Branding',
        'branding',
        'Brand identity, consistency, messaging, and market positioning',
        TRUE, 15.00, 4, 'Palette', '#F59E0B'
    ),
    (
        'cat-operations',
        'Operations',
        'operations',
        'Business operations efficiency, tools, workflows, and customer management',
        TRUE, 15.00, 5, 'Settings', '#EF4444'
    ),
    (
        'cat-ai-readiness',
        'AI Readiness',
        'ai-readiness',
        'Adoption of AI tools, automation potential, and readiness for AI-driven strategies',
        TRUE, 10.00, 6, 'Cpu', '#06B6D4'
    )
ON CONFLICT (id) DO NOTHING;


-- ============================================================
-- SECTION 3: Audit Subcriteria (24 subcriteria, 4 per category)
-- Source: seed-bi.sql
-- display_order column removed (dropped per audit; sort_order is canonical).
-- ============================================================

-- Web Presence
INSERT INTO audit_subcriteria (id, category_id, name, description, sort_order)
VALUES
    (
        'sub-web-design', 'cat-web-presence',
        'Visual Design & Layout',
        'Overall aesthetic quality, modern design patterns, visual hierarchy, and brand alignment',
        1
    ),
    (
        'sub-web-responsive', 'cat-web-presence',
        'Mobile Responsiveness',
        'Responsive behavior across devices, touch targets, mobile navigation, and viewport optimization',
        2
    ),
    (
        'sub-web-speed', 'cat-web-presence',
        'Page Speed & Performance',
        'Core Web Vitals (LCP, FID, CLS), load times, asset optimization, and server response',
        3
    ),
    (
        'sub-web-content', 'cat-web-presence',
        'Content Quality & Structure',
        'Content organization, readability, calls-to-action, and information architecture',
        4
    )
ON CONFLICT (id) DO NOTHING;
...
VALUES
    (
        'sub-seo-onpage', 'cat-seo',
        'On-Page SEO',
        'Title tags, meta descriptions, heading hierarchy, keyword usage, and internal linking',
        1
    ),
    (
        'sub-seo-technical', 'cat-seo',
        'Technical SEO',
        'Crawlability, indexation, sitemap, robots.txt, structured data, and canonical tags',
        2
    ),
    (
        'sub-seo-local', 'cat-seo',
        'Local SEO',
        'Google Business Profile, local citations, NAP consistency, reviews, and local keywords',
        3
    ),
    (
        'sub-seo-authority', 'cat-seo',
        'Content & Authority',
        'Blog presence, content freshness, backlink profile, domain authority, and topical relevance',
        4
    )
ON CONFLICT (id) DO NOTHING;
...
VALUES
    (
        'sub-social-presence', 'cat-social-media',
        'Platform Presence',
        'Active profiles on relevant platforms, profile completeness, and cross-platform consistency',
        1
    ),
    (
        'sub-social-strategy', 'cat-social-media',
        'Content Strategy',
        'Posting frequency, content mix, visual quality, hashtag usage, and content calendar',
        2
    ),
    (
        'sub-social-engage', 'cat-social-media',
        'Engagement & Community',
        'Follower interaction, response times, community building, and user-generated content',
        3
    ),
    (
        'sub-social-growth', 'cat-social-media',
        'Growth & Analytics',
        'Follower growth rate, reach trends, engagement metrics, and data-driven optimization',
        4
    )
ON CONFLICT (id) DO NOTHING;
...
VALUES
    (
        'sub-brand-visual', 'cat-branding',
        'Visual Identity',
        'Logo quality, color palette consistency, typography, and brand asset library',
        1
    ),
    (
        'sub-brand-messaging', 'cat-branding',
        'Messaging & Voice',
        'Brand voice consistency, tagline clarity, value proposition, and storytelling',
        2
    ),
    (
        'sub-brand-position', 'cat-branding',
        'Market Positioning',
        'Competitive differentiation, target audience alignment, and unique selling proposition',
        3
    ),
    (
        'sub-brand-consistent', 'cat-branding',
        'Brand Consistency',
        'Cross-channel consistency, style guide adherence, and professional presentation',
        4
    )
ON CONFLICT (id) DO NOTHING;
...
VALUES
    (
        'sub-ops-crm', 'cat-operations',
        'Customer Management',
        'CRM usage, customer communication tools, follow-up processes, and retention strategies',
        1
    ),
    (
        'sub-ops-tools', 'cat-operations',
        'Digital Tools & Software',
        'Tech stack evaluation, tool integration, workflow automation, and software utilization',
        2
    ),
    (
        'sub-ops-reviews', 'cat-operations',
        'Online Reviews & Reputation',
        'Review monitoring, response strategy, reputation management, and testimonial collection',
        3
    ),
    (
        'sub-ops-process', 'cat-operations',
        'Process Efficiency',
        'Standard operating procedures, team workflows, task management, and bottleneck identification',
        4
    )
ON CONFLICT (id) DO NOTHING;
...
VALUES
    (
        'sub-ai-adoption', 'cat-ai-readiness',
        'Current AI Adoption',
        'Existing AI tool usage, chatbots, automated responses, and AI-assisted content creation',
        1
    ),
    (
        'sub-ai-data', 'cat-ai-readiness',
        'Data Infrastructure',
        'Data collection practices, analytics setup, data quality, and integration readiness',
        2
    ),
    (
        'sub-ai-automation', 'cat-ai-readiness',
        'Automation Potential',
        'Identifiable automation opportunities, repetitive task analysis, and workflow optimization areas',
        3
    ),
    (
        'sub-ai-strategy', 'cat-ai-readiness',
        'AI Strategy Readiness',
        'Team AI literacy, willingness to adopt, budget allocation, and strategic AI implementation potential',
        4
    )
ON CONFLICT (id) DO NOTHING;
...
-- ============================================================
-- SECTION 4: Recommendation Templates (10 templates)
-- Source: seed-bi.sql
-- ON CONFLICT (id) DO NOTHING — safe to re-run.
-- ============================================================

INSERT INTO recommendation_templates
    (id, title, category_id, category, trigger_condition, description,
     expected_outcome, estimated_cost_min, estimated_cost_max,
     estimated_timeline, linked_service, priority, impact)
VALUES
    -- Web Presence
    (
        'tpl-web-redesign',
