...
router.delete('/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    ...
    if (cats[0].is_base) return res.status(403).json({ success: false, error: 'Cannot delete base category' });
...
router.post('/:id/subcriteria', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const { name, description, max_score, sort_order } = req.body;
    const id = generateId();
    await pool.query(
...
router.put('/subcriteria/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const { name, description, max_score, sort_order } = req.body;
    await pool.query(
...
router.delete('/subcriteria/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
