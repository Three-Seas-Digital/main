...
function computePeriodBounds(periodType, periodLabel) {
  let start, end;
  switch (periodType) {
    case 'daily': {
...
async function compileClientSnapshot(clientId, periodStart, periodEnd, periodType, localStorageData) {
  ...
  let auditScores = {};
  if (results[2].status === 'fulfilled' && results[2].value[0].length > 0) {
    const auditIds = results[2].value[0].map(a => a.id);
    try {
  ...
  if (results[6].status === 'fulfilled' && results[6].value[0].length > 0) {
    const intIds = results[6].value[0].map(i => i.id);
    try {
  ...
  const audits = rawAudits.map(a => ({ ...a, category_scores: auditScores[a.id] || [] }));
  ...
  const interventions = rawInterventions.map(i => ({ ...i, metrics: interventionMetrics[i.id] || [] }));
  ...
  const snapshotData = {
    ...
    execution_plans: executionPlans.map(ep => {
...
router.post('/clients/:clientId/snapshots', authenticateToken, requireRole('owner', 'admin', 'manager', 'analyst'), async (req, res) => {
  try {
    ...
    if (!label) {
      switch (period_type) {
        ...
        case 'weekly': {
          ...
          const wk = Math.ceil(((now - onejan) / 86400000 + onejan.getDay()) / 7);
...
router.get('/clients/:clientId/snapshots', authenticateToken, async (req, res) => {
  try {
    const { clientId } = req.params;
...
router.get('/clients/:clientId/snapshots/:snapshotId', authenticateToken, async (req, res) => {
  try {
...
router.post('/clients/:clientId/analyze', authenticateToken, requireRole('owner', 'admin', 'manager', 'analyst'), async (req, res) => {
  try {
    const { clientId } = req.params;
    ...
    if (!snapshotId) {
      ...
      await pool.query(
        ...
        [snapshotId, clientId, period_type, label, start, end,
         ...
         compiled.completenessScore, req.user.userId]
      );
      ...
      if (!rows.length) return res.status(404).json({ success: false, error: 'Snapshot not found' });
      snapshotData = typeof rows[0].snapshot_data === 'string'
        ? JSON.parse(rows[0].snapshot_data) : rows[0].snapshot_data;
    }
    ...
    await pool.query(
      ...
      [recId, clientId, snapshotId, analysis_type, req.user.userId]
    );
    ...
    const counts = { total: recs.length, critical: 0, high: 0, medium: 0, low: 0 };
    recs.forEach(r => { if (counts[r.priority] !== undefined) counts[r.priority]++; });
    ...
    res.json({
      ...
      data: {
        ...
        items: recs.map((r, i) => ({ ...r, display_order: i, admin_status: 'new' })),
      }
    });
  } catch (err) {
...
router.get('/clients/:clientId/analyses', authenticateToken, async (req, res) => {
  try {
    const [rows] = await pool.query(
...
router.get('/clients/:clientId/analyses/:analysisId', authenticateToken, async (req, res) => {
  try {
    const [analyses] = await pool.query(
      'SELECT * FROM ai_recommendations WHERE id = ? AND client_id = ?',
      [req.params.analysisId, req.params.clientId]
    );
    if (!analyses.length) return res.status(404).json({ success: false, error: 'Analysis not found' });
    ...
    res.json({ success: true, data: { ...analysis, items } });
  } catch (err) {
...
router.put('/clients/:clientId/analyses/:analysisId/items/:itemId', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
...
router.delete('/clients/:clientId/analyses/:analysisId', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    await pool.query(
...
router.get('/webhook/:clientId', authenticateWebhook, async (req, res) => {
  try {
    const { clientId } = req.params;
    ...
    const params = [clientId, period_type];
...
router.post('/webhook/:clientId', authenticateWebhook, async (req, res) => {
  try {
    const { clientId } = req.params;
    ...
    if (!snaps.length) return res.status(404).json({ success: false, error: 'Snapshot not found' });

    const counts = { total: recommendations.length, critical: 0, high: 0, medium: 0, low: 0 };
