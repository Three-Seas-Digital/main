...
router.get('/audits', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.get('/score-history', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.get('/recommendations', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.post('/recommendations/:id/accept', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.post('/recommendations/:id/decline', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
    const recId = req.params.id;
...
router.post('/recommendations/:id/thread', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
    const recId = req.params.id;
...
router.get('/recommendations/:id/threads', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
    const recId = req.params.id;
...
router.get('/metrics', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.get('/financials', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.get('/financials/revenue', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.get('/financials/expenses', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.get('/interventions', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.post('/service-requests', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
    ...
    if (recommendation_id) {
      ...
      if (recCheck.length === 0) {
        res.status(400).json({ success: false, error: 'Invalid recommendation reference' });
        return;
      }
    }
...
router.get('/service-requests', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.post('/feedback', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
    ...
    if (!validTargets.includes(target_type)) {
      res.status(400).json({ success: false, error: 'Invalid target_type. Must be: ' + validTargets.join(', ') });
      return;
    }
...
router.get('/feedback', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.get('/notifications', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.put('/notification-prefs', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
    const {
      email_digest,
...
router.put('/profile', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
...
router.put('/onboarding', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
    const { onboarding } = req.body;

    if (!onboarding || typeof onboarding !== 'object') {
      res.status(400).json({ success: false, error: 'onboarding object required' });
      return;
    }
    ...
    if (onboarding.documents && (current as any).documents) {
...
router.post('/projects', async (req: any, res: Response): Promise<void> => {
  try {
    const clientId = req.client?.clientId;
