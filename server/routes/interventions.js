...
router.post('/:clientId/interventions', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    ...
    if (!intervention_type || !VALID_TYPES.includes(intervention_type)) {
      return res.status(400).json({ success: false, error: `Invalid intervention type. Must be one of: ${VALID_TYPES.join(', ')}` });
    }
    if (status && !VALID_STATUSES.includes(status)) {
...
router.post('/:clientId/interventions/:id/metrics', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    ...
    if (baseline_value === undefined || baseline_value === null) {
      return res.status(400).json({ success: false, error: 'Baseline value is required' });
    }
...
router.post('/:clientId/interventions/:id/snapshots', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    ...
    if (metrics.length === 0) {
      return res.status(400).json({ success: false, error: 'No metrics to snapshot. Add metrics first.' });
    }
    ...
    if (intervention[0].implementation_date) {
      ...
      const now = new Date();
      daysSinceLaunch = Math.floor((now - launchDate) / (1000 * 60 * 60 * 24));
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const updates = {};
      if (req.files && req.files.before_screenshot && req.files.before_screenshot[0]) {
        updates.before_screenshot_url = `/uploads/documents/${req.files.before_screenshot[0].filename}`;
      }
      if (req.files && req.files.after_screenshot && req.files.after_screenshot[0]) {
        updates.after_screenshot_url = `/uploads/documents/${req.files.after_screenshot[0].filename}`;
      }

      if (Object.keys(updates).length === 0) {
        return res.status(400).json({ success: false, error: 'No screenshots provided' });
      }
