...
router.get('/', authenticateToken, requireRole('owner', 'admin', 'manager', 'it', 'analyst'), async (req, res) => {
  try {
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (rows.length === 0) return res.status(404).json({ error: 'Research not found' });
    res.json(rows[0]);
  } catch (err) {
    console.error('[research] Error:', err);
    res.status(500).json({ error: 'Server error' });
...
router.post('/', authenticateToken, requireRole('owner', 'admin', 'manager', 'it', 'analyst'), async (req, res) => {
  try {
    ...
    res.status(201).json({ id: result.insertId, message: 'Research saved' });
  } catch (err) {
    console.error('[research] Error:', err);
    res.status(500).json({ error: 'Server error' });
  }
...
router.put('/:id', authenticateToken, requireRole('owner', 'admin', 'manager', 'it', 'analyst'), async (req, res) => {
  try {
    const { location, data } = req.body;
    const lookupKey = `${location}`;
    await pool.query(
...
router.delete('/:id', authenticateToken, requireRole('owner', 'admin', 'manager', 'it', 'analyst'), async (req, res) => {
  try {
