...
router.post('/client/:clientId', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (audits.length === 0) return res.status(404).json({ success: false, error: 'Audit not found' });
...
router.post('/:id/publish', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    await pool.query(
...
router.post('/:id/scores', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const { scores } = req.body; // [{category_id, score, weight, internal_notes, client_summary, evidence_urls}]
    const auditId = req.params.id;

    if (!Array.isArray(scores) || scores.length === 0) {
      return res.status(400).json({ success: false, error: 'Scores array required' });
    }
...
router.post('/:id/subcriteria-scores', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
