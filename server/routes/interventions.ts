...
// ============================================================
router.get('/:clientId/interventions/summary', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    const totalInterventions = statusRows.reduce((sum: number, r: any) => sum + r.count, 0);

    // Success rate: completed+measured / total that are not planned
    const completed = statusRows
      .filter((r: any) => r.status === 'completed' || r.status === 'measured')
      .reduce((sum: number, r: any) => sum + r.count, 0);
    const attempted = statusRows
      .filter((r: any) => r.status !== 'planned')
      .reduce((sum: number, r: any) => sum + r.count, 0);
...
// ============================================================
router.get('/:clientId/interventions', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { clientId } = req.params;
...
// ============================================================
router.post('/:clientId/interventions', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req: any, res: Response): Promise<void> => {
  try {
    const { clientId } = req.params;
    const {
...
// ============================================================
router.put('/:clientId/interventions/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req: any, res: Response): Promise<void> => {
  try {
...
// ============================================================
router.delete('/:clientId/interventions/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req: any, res: Response): Promise<void> => {
  try {
    const { clientId, id } = req.params;
...
// ============================================================
router.post('/:clientId/interventions/:id/metrics', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req: any, res: Response): Promise<void> => {
  try {
    const { clientId, id } = req.params;
    const {
...
// ============================================================
router.put('/:clientId/interventions/:id/metrics/:metricId', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req: any, res: Response): Promise<void> => {
  try {
...
// ============================================================
router.post('/:clientId/interventions/:id/snapshots', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req: any, res: Response): Promise<void> => {
  try {
    const { clientId, id } = req.params;
...
// ============================================================
router.get('/:clientId/interventions/:id/snapshots', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { clientId, id } = req.params;
