  ...
  try {
    ...
    if (clientId) {
      ...
      if (clientCheck.length === 0) {
        return res.status(400).json({ error: 'Client not found — cannot create invoice for non-existent client' });
      }
    }
  ...
  try {
    ...
    if (clients.length > 0 && clients[0].email) {
      sendInvoiceEmail(invoice, clients[0], { method: paymentMethod || 'other', created_at: new Date().toISOString() })
        .catch(err => console.error('[invoices] Email send failed:', err.message));
    }
