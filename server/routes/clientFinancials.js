...
router.get('/:clientId/financials', authenticateToken, async (req, res) => {
  try {
...
router.post('/:clientId/financials', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const { clientId } = req.params;
    const {
      period_year,
    ...
    if (!period_year || !period_month) {
      return res.status(400).json({ success: false, error: 'Period year and month are required' });
    }
...
router.put('/:clientId/financials/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
...
router.delete('/:clientId/financials/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const { id, clientId } = req.params;
...
router.get('/:clientId/financials/summary', authenticateToken, async (req, res) => {
  try {
    const { clientId } = req.params;
    const { startYear, startMonth, endYear, endMonth } = req.query;
...
router.get('/:clientId/financials/channels', authenticateToken, async (req, res) => {
  try {
    const { clientId } = req.params;
    const { startYear, startMonth, endYear, endMonth } = req.query;
...
router.get('/:clientId/financials/products', authenticateToken, async (req, res) => {
  try {
    const { clientId } = req.params;
    const { startYear, startMonth, endYear, endMonth } = req.query;
...
router.get('/:clientId/financials/ad-spend', authenticateToken, async (req, res) => {
  try {
    const { clientId } = req.params;
    const { startYear, startMonth, endYear, endMonth } = req.query;
...
router.post('/:clientId/financials/ad-spend', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const { clientId } = req.params;
    const {
      platform,
...
router.put('/:clientId/financials/ad-spend/:id', authenticateToken, requireRole('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const { id, clientId } = req.params;
    const {
      spend,
    ...
    if (existing.length === 0) {
      return res.status(404).json({ success: false, error: 'Ad spend record not found' });
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // CSV has: date, revenue, expenses, customers, conversion_rate, avg_revenue_per_customer, new_customers, churn_rate, notes
      // DB expects: period_year, period_month, gross_revenue, net_revenue, etc.
      const insertPromises = valid.map((record) => {
